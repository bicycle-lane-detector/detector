...
def splitSource(source, destImg, destMasks):
    ...
    for n, file in enumerate(tqdm(files, total=len(files))):
        if n % 2 == 0:
            os.rename(source + file, destMasks + file[:-9] + ".png")
        else:
            os.rename(source + file, destImg + file[:-8] + ".jpg")
...
if __name__ == "__main__":
    ...
    new_images_path = root_data_path + "Images256/"
    new_masks_path = root_data_path + "Masks256/"
    ...
    splitSource("./DeepGlobe/source_train/", "./DeepGlobe/Images/", "./DeepGlobe/Masks/")
